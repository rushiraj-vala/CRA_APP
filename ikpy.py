...
VTCP = [600, -100, 600, -90, 0, -179]  # Ejemplo
...
# # the robotiq gripper inside RoboDK, It is not TCP attached to robot
# dt_gripper = RDK.Item('RobotiQ 2F85')
...
d4 = 174.15  # En mm
d5 = 119.85  # En mm
d6 = 116.55  # En mm. Se suma el valor del TCP de la herramienta
...
def CInversa(VTCP):
    ...
    T = np.array([[cos(phi)*cos(Theta), (sen(Psi)*cos(phi)*sen(Theta))-(cos(Psi)*sen(phi)), (sen(Psi)*sen(phi))+(cos(Psi)*cos(phi)*sen(Theta)), PX],
                  [cos(Theta)*sen(phi), (cos(Psi)*cos(phi))+(sen(Psi)*sen(phi)*sen(Theta)),
                   (cos(Psi)*sen(phi)*sen(Theta))-(sen(Psi)*cos(phi)), PY],
    ...
    # Se resta a la posicion en el SC6 la distancia al SC5 considerando la rotacion Z06
    P05 = P06-np.dot(d6, Z06)
    print(P05)
    phi1 = math.degrees(math.atan2(P05[1], P05[0]))
    P05XY = math.sqrt(math.pow(P05[1], 2)+math.pow(P05[0], 2))
    print(d4, P05XY, (d4/P05XY))
    ...
    theta5 = math.degrees(
        math.acos((P06[0]*sen(theta1)-P06[1]*cos(theta1)-d4)/d6))
    ...
    T1 = np.array([[cos(theta1), 0, sen(theta1), 0],
                   [sen(theta1), 0, -cos(theta1), 0],
                   [0, 1, 0, d1],
                   [0, 0, 0, 1]])

    T5 = np.array([[cos(theta5), 0, -sen(theta5), 0],
                   [sen(theta5), 0, cos(theta5), 0],
                   [0, -1, 0, d5],
                   [0, 0, 0, 1]])

    T6 = np.array([[cos(theta6), -sen(theta6), 0, 0],
                   [sen(theta6), cos(theta6), 0, 0],
                   [0, 0, 1, d6],
                   [0, 0, 0, 1]])
    ...
    theta2 = (math.degrees(
        math.atan2(-P14[1][3], -P14[0][3])))-math.degrees(math.asin(-a3*sen(theta3)/P14XY))
    T2 = np.array([[cos(theta2), -sen(theta2), 0, a2*cos(theta2)], [sen(theta2),
                  cos(theta2), 0, a2*sen(theta2)], [0, 0, 1, 0], [0, 0, 0, 1]])
    T3 = np.array([[cos(theta3), -sen(theta3), 0, a3*cos(theta3)], [sen(theta3),
                  cos(theta3), 0, a3*sen(theta3)], [0, 0, 1, 0], [0, 0, 0, 1]])
    P12 = T2  # CAMBIO
    P23 = T3  # CAMBIO
    P13 = np.dot(P12, P23)
